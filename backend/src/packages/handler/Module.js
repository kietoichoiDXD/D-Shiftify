...
import { SENTRY_DSN } from 'core/env';
import { sendErrorToSentry } from 'core/utils';
...
export class Module {
        ...
        return async (req, res, next) => {
            ...
            if (!canActive) {
                if (guardClass.getMessage) {
                    return next(
                        new ForbiddenException(guardClass.getMessage()),
                    );
                }
                return next(
    ...
    static #produceInterceptor = interceptorClass => {
        if (!interceptorClass['intercept']) {
            throw new MethodRequired(
                interceptorClass.constructor.name,
                'intercept',
            );
        }
